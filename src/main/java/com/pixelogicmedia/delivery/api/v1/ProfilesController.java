...
public class ProfilesController extends BaseController implements ProfilesApi {
    ...
    @Override
    public ResponseEntity<ProfileResource> createProfile(final CreateProfileResource profileResource) {
    ...
    @Override
    public ResponseEntity<ProfileResource> updateProfile(final Long id, final CreateProfileResource profileResource) {
        final var profile = this.profileService.updateProfile(id, this.profileMapper.map(profileResource));
        return ResponseEntity.ok(this.profileMapper.map(profile));
    }
}
